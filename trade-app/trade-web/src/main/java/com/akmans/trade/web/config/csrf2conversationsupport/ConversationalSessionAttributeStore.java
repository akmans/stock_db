...
public class ConversationalSessionAttributeStore implements SessionAttributeStore, InitializingBean {
	...
	private org.slf4j.Logger logger = LoggerFactory.getLogger(ConversationalSessionAttributeStore.class.getName());
	...
	public Object retrieveAttribute(WebRequest request, String attributeName) {
		...
		if (getConversationId(request) != null) {
			if (logger.isDebugEnabled()) {
				logger.debug("retrieveAttribute - retrieving bean reference for (" + attributeName
						+ ") for conversation (" + getConversationId(request) + ").");
			}
